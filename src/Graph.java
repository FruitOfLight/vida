...
=======
	...
	public void draw(Graphics g) {
		...
		if (begin != null && !GUI.gkl.isPressed(CONST.deleteKey)
				&& !GUI.gkl.isPressed(CONST.moveKey)) {
			g.setColor(new Color(0, 0, 0));
			g.drawLine((int) (offX + begin.getX() * zoom),
	...
	public void createEdge(Vertex from, Vertex to) {
		...
		// radius2)
		repaintBetween((int) from.getX(), (int) from.getY(), (int) to.getX(),
				(int) to.getY());
	}
	...
	public void repaintBetween(int x1, int y1, int x2, int y2) {
		...
		canvas.repaint(x1 - CONST.vertexSize, y1 - CONST.vertexSize, x2 - x1
				+ CONST.vertexSize * 2, y2 - y1 + CONST.vertexSize * 2);
	}
	...
	public void read(Scanner input) {
		...
		edges = new ArrayList<Edge>();
		try {
			int n = Integer.parseInt(input.next()), m = Integer.parseInt(input
					.next());
			for (int i = 0; i < n; i++) {
				double x = Double.parseDouble(input.next()), y = Double
						.parseDouble(input.next());
				int ID = Integer.parseInt(input.next());
				vertices.add(new Vertex(x, y, ID));
			}
			for (int i = 0; i < m; i++) {
				int f = Integer.parseInt(input.next()), t = Integer
						.parseInt(input.next());
				createEdge(vertices.get(f), vertices.get(t));
			}
		} catch (Exception e) {
	...
	private void createClique(int n, boolean edges) {
		...
		for (int i = 0; i < n; ++i)
			createVertex(d * Math.sin(i * 2 * Math.PI / n),
					-d * Math.cos(i * 2 * Math.PI / n), getNewVertexID());
		if (edges)
			for (int i = 0; i < n; ++i)
				for (int j = i + 1; j < n; ++j)
					createEdge(vertices.get(i), vertices.get(j));
	...
	private void createGrid(int m, int n, boolean edges) {
		...
		if (edges)
			for (int i = 0; i < m; i++)
				for (int j = 0; j < n; j++) {
					if (j != n - 1)
						createEdge(vertices.get(i * n + j),
								vertices.get(i * n + j + 1));
					if (i != m - 1)
						createEdge(vertices.get(i * n + j),
								vertices.get(i * n + j + n));
				}
	}
>>>>>>> 53013db2a5e9af6644a19415a73de16d4e9af457
}
